...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    if (isset($_POST['email'])) {
        ...
        if (mysqli_num_rows($emailResult) > 0) {
            ...
            $updateQuery = "UPDATE user SET verified_code='$otp' WHERE email='$email'";
            if (mysqli_query($koneksi, $updateQuery)) {
                ...
                try {
                    ...
                    $mail->setFrom('otp@verif.com', 'Verification Reset Password');
                    $mail->addAddress($email);

                    $mail->isHTML(true);
                    $mail->Subject = 'Verification Reset Password [' . $otp . ']';
