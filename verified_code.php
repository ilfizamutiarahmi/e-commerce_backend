...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    if (isset($_POST['email']) && isset($_POST['verified_code'])) {

        $email = $_POST['email'];
        $otp = $_POST['verified_code'];

        $email = mysqli_real_escape_string($koneksi, $email);
        $otp = mysqli_real_escape_string($koneksi, $otp);

        $query = "SELECT * FROM user WHERE email='$email' AND verified_code='$otp'";
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            $updateQuery = "UPDATE user SET is_verified = 'verified' WHERE email='$email'";
